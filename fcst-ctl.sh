...
if [ $SPINUP_FLAG == 1 ]; then
    ...
    NODELIST='nodelist.spinup'
else
    ARCHIVE_FLAG=1
    LOG_SUFFIX='.fcst.'
    JOB_NAME=njord
    NODELIST='nodelist.fcst'
fi
...
cd ${NJORD_ROOT} 
mpirun -iface=ib0 -f ${NODELIST} -np ${NTASKS_ALL} ./coawstM ./Projects/Njord/${CPL_IN}
EOF

echo ">>Run COAWST..."
if [ ${TEST_FLAG} == 0 ]; then
    sh run.sh >& ${NJORD_ROOT}/cwstv3${LOG_SUFFIX}${STRT_DATE_PACK}.log
fi
date
